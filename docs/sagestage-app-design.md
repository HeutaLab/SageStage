...
| seeding | four seeds in demo.js: the three-screen **tour** (no URL param) and three themed decks the landing cards deep-link — `try/?deck=maths` ("Maths in your hands", 2 screens), `?deck=english` ("English on the board", 3 screens: story map + word bank, then modelled writing alone, then the sentence builder alone at 1180x540 — it shared the writing screen at 540x470 until 2026-09-08, where its sentence line measured zero visible pixels), `?deck=everyday` ("Classroom management": the running-classroom screen, then noise/rewards/groups). Fresh browser: the chosen deck is the first state and boot lands on its screen 1. Returning browser: the app **adds the requested deck if missing and opens it** — existing work is never overwritten (added 2026-08-01 after Glenn's review). Widgets build from their own `defaults()` at boot |
| seed geometry | (2026-08-02, after the first layouts shipped four half-size widgets per screen and the teaching clock's face rendered at 0 height) every screen is authored on a **1280×720 reference canvas**, content inside y 72–630 / x 24–1256 so the topbar, toolbar dock and pager stay clear; **widgets keep ≥85% of their designed default size** — a manipulative that can't fit at near-full size goes in a later screen or stays in the tray, never ships crushed. demo.js maps the reference to the visitor's window once at seed time (positions per-axis, sizes by the smaller factor, clamped 0.8–1.7, centred when clamped) so one authored layout fits a small laptop and fills a classroom board. The teaching clock ships slim in the tour (`gameOn:false, quick:false` — the face IS the widget) and full-fat in the maths deck |
